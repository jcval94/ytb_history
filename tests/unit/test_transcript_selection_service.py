...
def _seed_inputs(root: Path) -> None:
    _write_csv(root / "decision/latest_action_candidates.csv", ["video_id", "channel_id", "channel_name", "channel_url", "title", "upload_date", "decision_score", "alpha_score", "metric_confidence_score"], [
        ["v1", "c1", "bilinkis", "https://www.youtube.com/@bilinkis", "T1", "2026-04-20T00:00:00+00:00", "80", "70", "60"],
        ["v2", "c2", "veritasium", "https://www.youtube.com/veritasium", "T2", "2026-04-21T00:00:00+00:00", "90", "70", "60"],
        ["v3", "c3", "other", "https://www.youtube.com/@other", "T3", "2026-04-22T00:00:00+00:00", "70", "70", "60"],
        ["v4", "c4", "other", "https://www.youtube.com/@other", "T4", "2026-04-23T00:00:00+00:00", "60", "70", "60"],
    ])
    ...
    _write_csv(root / "analytics/latest/latest_video_metrics.csv", ["video_id", "channel_id", "channel_name", "channel_url", "title", "upload_date"], [["v1", "c1", "bilinkis", "https://www.youtube.com/@bilinkis", "T1", "2026-04-20T00:00:00+00:00"], ["v2", "c2", "veritasium", "https://www.youtube.com/veritasium", "T2", "2026-04-21T00:00:00+00:00"], ["v3", "c3", "other", "https://www.youtube.com/@other", "T3", "2026-04-22T00:00:00+00:00"], ["v4", "c4", "other", "https://www.youtube.com/@other", "T4", "2026-04-23T00:00:00+00:00"]])
...
def test_forced_channels_and_ranked_behavior(tmp_path: Path) -> None:
    ...
    assert report["transcript_queue_path"].endswith("transcript_queue.jsonl")
    assert report["selection_report_path"].endswith("transcript_selection_report.json")
    assert queue[0]["selection_source"] == "forced_channel_new_video"
    assert queue[0]["video_id"] == "v1"
    assert queue[0]["channel_url"] == "https://www.youtube.com/@bilinkis"
...
def test_recent_failed_transcription_enters_cooldown(tmp_path: Path) -> None:
    _seed_inputs(tmp_path)
    now = datetime.now(timezone.utc)
    _write_jsonl(
...
def test_failed_transcription_without_failed_at_uses_selected_at_cooldown(tmp_path: Path) -> None:
    _seed_inputs(tmp_path)
    now = datetime.now(timezone.utc)
    _write_jsonl(
        tmp_path / "transcripts/transcript_registry.jsonl",
