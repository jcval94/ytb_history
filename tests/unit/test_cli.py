...
def test_cli_transcribe_selected_videos_forwards_local_audio_options(monkeypatch, capsys) -> None:
    calls: list[dict] = []
    ...
    assert calls == [
        {
            ...
            "ytdlp_extra_args": ["--force-ipv4", "--socket-timeout", "30"],
            "ytdlp_cookies_b64": "Y29va2llcw==",
            "segment_large_audio": False,
        }
    ]
...
def test_cli_sync_local_repo_prints_json(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["ytb_history", "sync-local-repo", "--repo-dir", "/repo", "--remote", "origin", "--branch", "main", "--check-only"],
    )
    ...
    assert calls == [{"repo_dir": "/repo", "remote": "origin", "branch": "main", "check_only": True}]


def test_cli_diagnose_local_transcription_prints_json(monkeypatch, capsys) -> None:
    calls: list[dict] = []

    def _fake_diagnose_local_transcription(**kwargs):
        calls.append(kwargs)
        return {"status": "success", "task_name": kwargs["task_name"]}

    monkeypatch.setattr(cli, "diagnose_local_transcription", _fake_diagnose_local_transcription)
    monkeypatch.setattr(
        "sys.argv",
        [
            "ytb_history",
    ...
    assert code == 0
    assert json.loads(out) == {"status": "success", "task_name": "Task"}
    assert calls == [
        {
            "repo_dir": "/repo",
            "data_dir": "custom/data",
