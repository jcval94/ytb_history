param(
    ...
    [string]$PythonPath = "",
    [int]$Limit = 10,
    [string]$ShortcutName = "YTB History - Play Local Automation"
)
...
$arguments = '-NoProfile -ExecutionPolicy Bypass -File "{0}" -RepoDir "{1}" -PythonPath "{2}" -Limit {3} -PauseOnExit' -f $playRunnerPath, $repoRoot, $PythonPath, $Limit
...
$shortcut.Description = "Run local repo sync, transcription, and transcript publication for ytb_history."
$shortcut.IconLocation = "$powershellPath,0"
$shortcut.Save()

$automationDir = Join-Path $repoRoot "build\local_automation"
New-Item -ItemType Directory -Force -Path $automationDir | Out-Null
$reportPath = Join-Path $automationDir "latest_play_shortcut.json"
$report = [ordered]@{
...
Write-Output "Desktop play shortcut created: $shortcutPath"
